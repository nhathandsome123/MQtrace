...
def process_mqtt_event(event, write_api):
    ...
    if mqtt_type == "publish":
        # Suricata thường dùng payload_printable hoặc message hơn là payload
        raw_payload = (
            flat_fields.get("payload")
            or flat_fields.get("payload_printable")
            or flat_fields.get("message")
            or ""
        )
        ...
        cleaned_payload = clean_payload_for_json(raw_payload)
        payload_bytes = cleaned_payload.encode("utf-8", errors="replace")
        ...
        if payload_len > 0:
            ...
            # Nếu payload là JSON, flatten 1 level -> p_<key>
            try:
